...
export class PeliculaEditController {
  ...
  $onInit() {
    if (this.stateParams.id != null) {
      this.peliculaApiService
        .getPeliculaId(this.stateParams.id)
        .then((result) => {
  ...
  handleClick = (
    ...
    if (this.stateParams.id != null) {
      ...
      this.peliculaApiService.updatePelicula(this.pelicula).then((result) => {
      ...
      this.peliculaApiService.insertPelicula(this.pelicula).then((result) => {
        window.location.replace("http://localhost:8080/#!/home");
      });
    }
  };
